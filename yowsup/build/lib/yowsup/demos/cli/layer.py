...
from yowsup.common.tools import Jid, StorageTools
...
class YowsupCliLayer(Cli, YowInterfaceLayer):
    ...
    def onMessage(self, message):
        ...
        def fix_encoding(msg):
            """
            Return `msg` with its encoding fixed if possible.
            Otherwise return `msg`.
            """
            try:
                if sys.version_info >= (3, 0):
                    return msg.encode('latin-1').decode()
                else:
                    return msg.encode('utf-8')
            except Exception as e:
                logger.info('Encoding already fixed:%s', e)
                return msg
        ...
        output = self.__class__.MESSAGE_FORMAT.format(
            FROM = sender,
            TIME = formattedDate,
            MESSAGE = fix_encoding(messageOut),
    ...
    def getDownloadableMediaMessageBody(self, message):
        ...
        return "[Media Type:{media_type}, Size:{media_size}, URL:{media_url}, Saved Path:{saved_path}]".format(
            media_type = message.getMediaType(),
            media_size = message.getMediaSize(),
            media_url = message.getMediaUrl(),
            saved_path = save_as(message.getMediaContent(), message.getId())
        )
